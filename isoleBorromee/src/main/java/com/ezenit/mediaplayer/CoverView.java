...
/**
 * Displays a flingable/draggable View of cover art/guide info images
...
public final class CoverView extends View implements Handler.Callback {
	 ...
	 */
	private AudioGuide[] mAudioGuides = new AudioGuide[3];
	/**
	...
	/**
	 * Cover art to use when a guide has no cover art in no info display styles.
	 */
	...
	/**
	 * If true, queryAudioGuides was called before the view initialized and should
	 ...
	 * @param callback The callback for nextAudioGuide/previousAudioGuide
	...
	protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight)
	{
		if (mPendingQuery && width != 0 && height != 0) {
			mPendingQuery = false;
			queryAudioGuides(PlaybackService.get(getContext()));
		}
	}
	...
	public boolean onTouchEvent(MotionEvent ev)
	{
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (Math.abs(deltaX) > Math.abs(deltaY)) {
				if (deltaX < 0) {
					int availableToScroll = scrollX - (mAudioGuides[0] == null ? width : 0);
		...
		case MotionEvent.ACTION_UP: {
			...
			// If -1 or 1, play the previous or next guide, respectively and scroll
			// to that guide's cover. If 0, just scroll back to current guide's cover.
			int whichCover = 0;
			int min = mAudioGuides[0] == null ? 0 : -1;
			int max = 1;
			...
			if (whichCover != 0) {
				...
				mCallback.shiftCurrentAudioGuide(whichCover);
				mScrollX = scrollX;
			}
	...
	/**
	 * Generates a bitmap for the given guide.
	 *
	 * @param i The position of the guide in mAudioGuides.
	 */
	private void generateBitmap(int i)
	{
		AudioGuide guide = mAudioGuides[i];

		int style = mCoverStyle;
		Context context = getContext();
		Bitmap cover = guide == null ? null : guide.getCover(context,getWidth(),getHeight());
		...
		if (cover == null && style == CoverBitmap.STYLE_NO_INFO) {
			...
			mBitmaps[i] = CoverBitmap.createBitmap(context, style, cover, guide, getWidth(), getHeight());
		}
	...
	/**
	 * Set the AudioGuide at position <code>i</code> to <code>guide</code>, generating
	 * the bitmap for it in the background if needed.
	 */
	public void setAudioGuide(int i, AudioGuide guide)
	{
		if (guide == mAudioGuides[i])
			return;

		mAudioGuides[i] = guide;
		mBitmaps[i] = null;
		if (mHandler != null) {
	 ...
	 */
	public void queryAudioGuides(PlaybackService service)
	{
		...
		AudioGuide[] songs = mAudioGuides;
		Bitmap[] bitmaps = mBitmaps;
		AudioGuide[] newAudioGuides = { service.getAudioGuide(-1), service.getAudioGuide(0), service.getAudioGuide(1) };
		Bitmap[] newBitmaps = new Bitmap[3];
		mAudioGuides = newAudioGuides;
		mBitmaps = newBitmaps;
		if (!mScrolling)
			mActiveBitmaps = newBitmaps;

		for (int i = 0; i != 3; ++i) {
			if (newAudioGuides[i] == null)
				continue;

			for (int j = 0; j != 3; ++j) {
				if (newAudioGuides[i] == songs[j]) {
					newBitmaps[i] = bitmaps[j];
					break;
				}
			}

			if (newBitmaps[i] == null&&mHandler!=null) {
				mHandler.sendMessage(mHandler.obtainMessage(MSG_GENERATE_BITMAP, i, 0));
			}
		}
	...
	/**
	 * Call {@link CoverView#generateBitmap(int)} for the guide at the given index.
	 *
	 * arg1 should be the index of the guide.
	 */
