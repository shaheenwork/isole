...
import com.ezenit.download.utils.ConfigUtils;
import com.ezenit.download.utils.DownloaderIntents;
import com.ezenit.download.utils.NetworkUtils;
import com.ezenit.download.utils.StorageUtils;
...
public class DownloadManager extends Thread {
	...
	public static final String ACTION_DOWNLOAD_STATUS = "com.ezenit.download.services.DownloadManager.ACTION_DOWNLOAD_STATUS";
    ...
    public void addTask(String url,String destFolder) {

        if (!StorageUtils.isSDCardPresent()) {
            Toast.makeText(mContext, "SD Card is not present", Toast.LENGTH_LONG).show();
            return;
        }

        if (!StorageUtils.isSdCardWrittenable()) {
            Toast.makeText(mContext, "SD card is write protected", Toast.LENGTH_LONG).show();
            return;
        }

        if (getTotalTaskCount() >= MAX_TASK_COUNT) {
            Toast.makeText(mContext, "Only "+MAX_TASK_COUNT+" downloads can run simultaneously", Toast.LENGTH_LONG).show();
            return;
        }

        try {
            addTask(newDownloadTask(url,destFolder));
    ...
    private void broadcastAddTask(String url, boolean isInterrupt) {
        ...
        Intent nofityIntent = new Intent(ACTION_DOWNLOAD_STATUS);
        nofityIntent.putExtra(DownloaderIntents.TYPE, DownloaderIntents.Types.ADD);
        nofityIntent.putExtra(DownloaderIntents.URL, url);
        nofityIntent.putExtra(DownloaderIntents.IS_PAUSED, isInterrupt);
...
//        }
    ...
    public synchronized void deleteTask(String url) {
        ...
        for (int i = 0; i < mDownloadingTasks.size(); i++) {
            task = mDownloadingTasks.get(i);
            if (task != null && task.getUrl().equals(url)) {
                File file = new File(task.getFolderPath()
    ...
    public synchronized void pauseTask(DownloadTask task) {
        ...
        if (task != null) {
            ...
            try {
                ...
                task = newDownloadTask(url,task.getFolderPath());
                mPausingTasks.add(task);
    ...
    public synchronized void continueTask(DownloadTask task) {
        ...
        if (task != null) {
            ...
            mTaskQueue.offer(task);
            Intent nofityIntent = new Intent(ACTION_DOWNLOAD_STATUS);
            nofityIntent.putExtra(DownloaderIntents.TYPE, DownloaderIntents.Types.CONTINUE);
            nofityIntent.putExtra(DownloaderIntents.URL, task.getUrl());
            mContext.sendBroadcast(nofityIntent);
        }
    }
    ...
    public synchronized void completeTask(DownloadTask task) {
        ...
        if (mDownloadingTasks.contains(task)) {
            ...
            // notify list changed
            Intent nofityIntent = new Intent(ACTION_DOWNLOAD_STATUS);
            nofityIntent.putExtra(DownloaderIntents.TYPE, DownloaderIntents.Types.COMPLETE);
            nofityIntent.putExtra(DownloaderIntents.URL, task.getUrl());
            mContext.sendBroadcast(nofityIntent);
        }
    }
    
    public synchronized void clearTask(DownloadTask task) {
        if (mDownloadingTasks.contains(task)) {
            ConfigUtils.clearURL(mContext, mDownloadingTasks.indexOf(task));
            mDownloadingTasks.remove(task);
        }
     ...
     */
    private DownloadTask newDownloadTask(String url,String destFolder) throws MalformedURLException {
        ...
        DownloadTaskListener taskListener = new DownloadTaskListener() {
            ...
            public void updateProcess(DownloadTask task) {
                ...
                updateIntent.putExtra(DownloaderIntents.TYPE, DownloaderIntents.Types.PROCESS);
                updateIntent.putExtra(DownloaderIntents.PROCESS_SPEED, task.getDownloadSpeed() + "kbps | "
                        + task.getDownloadSize() + " / " + task.getTotalSize());
                updateIntent.putExtra(DownloaderIntents.PROCESS_PROGRESS, task.getDownloadPercent() + "");
                updateIntent.putExtra(DownloaderIntents.URL, task.getUrl());
            ...
            public void errorDownload(DownloadTask task, Throwable error) {
                ...
                if (error != null) {
                    Toast.makeText(mContext, "Error: " + error.getMessage(), Toast.LENGTH_LONG)
                            .show();
                    Intent errorIntent = new Intent(ACTION_DOWNLOAD_STATUS);
                    errorIntent.putExtra(DownloaderIntents.TYPE, DownloaderIntents.Types.ERROR);
                    errorIntent.putExtra(DownloaderIntents.ERROR_CODE, error);
                    errorIntent.putExtra(DownloaderIntents.ERROR_INFO,
                   		 error.getMessage());
                    errorIntent.putExtra(DownloaderIntents.URL, task.getUrl());
...
//                 }
        ...
        return new DownloadTask(mContext, url, destFolder, taskListener);
    }
